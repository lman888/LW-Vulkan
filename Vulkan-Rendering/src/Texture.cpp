...
void Texture::CreateTextureImage(const std::string texture)
{
    int textureWidth = 0;
    int textureHeight = 0;
    ...
    for (ModelLoader& models : *VulkanCore::GetModels())
    {
        ...
        CreateImage(textureWidth, textureHeight, m_mipLevels, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
...
void Texture::CreateTextureSampler()
{
    ...
    if (vkCreateSampler(VulkanCore::GetChosenDevice()->GetChosenGPUDevice(), &samplerInfo, nullptr, &m_textureSampler) != VK_SUCCESS)
    {
        throw std::runtime_error("Failed to create Texture Sampler!");
    }
}

VkImageView Texture::CreateImageView(VkImage& image, VkFormat format, VkImageAspectFlags flags, uint32_t mipLevels)
{
...
void Texture::CreateDepthResource()
{
    ...
    CreateImage(VulkanCore::GetSwapChain()->GetSwapChainImageExtent().width, VulkanCore::GetSwapChain()->GetSwapChainImageExtent().height, 1, depthFormat,
        VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_depthImage,
        m_depthImageMemory);
...
void Texture::CleanUp()
{
    ...
    vkDestroyImageView(VulkanCore::GetChosenDevice()->GetChosenGPUDevice(), m_textureImageView, nullptr);

    vkDestroyImage(VulkanCore::GetChosenDevice()->GetChosenGPUDevice(), m_textureImage, nullptr);
    vkFreeMemory(VulkanCore::GetChosenDevice()->GetChosenGPUDevice(), m_textureImageMemory, nullptr);
}
